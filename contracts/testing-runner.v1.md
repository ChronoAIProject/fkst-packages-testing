...
- `evidence_bundle`: optional `testing-runner.native-evidence-pointers.v1` table for `fkst-native`, containing only artifact paths under `artifact_root`.

`fkst-native` writes `.testing/runs/.../metadata.json` with schema `testing-runner.native-metadata.v1` for native results. Metadata includes `job`, `status`, `artifact_root`, `source_ref`, `trace_id`, `dedup_key`, bounded `adapter` metadata, optional `native_summary`, and optional `evidence_bundle` pointers. Artifact write failure must return a `blocked` result with an `artifact write failed` stderr excerpt.

`fkst-native` also writes a deterministic native evidence bundle under `<artifact_root>/evidence/` with schema `testing-runner.native-evidence-bundle.v1`. The bundle manifest is `<artifact_root>/evidence/bundle.json`; section files are `<artifact_root>/evidence/discovery.json`, `planning.json`, `execution.json`, `skipped.json`, `failures.json`, `actions.json`, `urls.json`, `observations.json`, `console.json`, `network.json`, `trace.json`, `screenshots.json`, and `dom_state.json`. The result event and metadata carry only the `testing-runner.native-evidence-pointers.v1` pointer table for those files. Section files may contain bounded summaries and artifact paths for browser actions, observations, sanitized URLs, console/network summaries, execution traces, screenshot pointers, and DOM/state pointers when available; they must not contain browser storage, credentials, cookies, tokens, screenshot bytes, report bodies, raw stdout/stderr bodies, or arbitrary nested browser data.
