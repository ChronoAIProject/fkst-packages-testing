...
`native_summary` is diagnostic metadata, not a downstream consumption requirement. Accepted nested native summaries are bounded `testing-runner.module-no-browser-summary.v1`, `testing-runner.online-heartbeat-summary.v1`, `testing-runner.browser-driver-summary.v1`, and `testing-runner.browser-exploration-summary.v1` payloads. Browser driver summaries may include only a readiness audit: `readiness.status` plus up to 16 session `{ role, status }` entries. Browser exploration summaries may include only bounded action records with `intent`, `action`, `target`, sanitized `url`, `priority`, `result`, `classification`, `observation`, and `evidence_pointer`; evidence bodies stay in artifacts. Readiness `checks`, report bodies, browser state, credentials, cookies, tokens, screenshots, traces, and arbitrary nested trees must be rejected.
