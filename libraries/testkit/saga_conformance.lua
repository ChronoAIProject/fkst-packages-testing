...
local function assert_delivery_succeeded(label, ok, result_or_err)
  if not ok then
    error(
      "testkit.saga_conformance: conformance-input-invalid: "
        .. label
...
local function validate_case(name, case)
  if type(case) ~= "table" then
    error("testkit.saga_conformance: conformance-input-invalid: " .. name .. " requires case")
  end
end
...
local function assert_non_empty_table(name, value)
  if type(value) ~= "table" or #value == 0 then
    error("testkit.saga_conformance: conformance-input-invalid: " .. name .. " requires non-empty table")
  end
end

local function validate_post_condition(step, condition)
  if type(condition) ~= "table" then
    error("testkit.saga_conformance: conformance-input-invalid: step " .. tostring(step.id) .. " has malformed post_condition")
  end
  if not is_non_empty_string(condition.id) then
    error("testkit.saga_conformance: conformance-input-invalid: step " .. tostring(step.id) .. " post_condition requires id")
  end
  if not is_non_empty_string(condition.kind) then
    error("testkit.saga_conformance: conformance-input-invalid: post_condition " .. tostring(condition.id) .. " requires kind")
  end
end

local function validate_step(step)
  if type(step) ~= "table" then
    error("testkit.saga_conformance: conformance-input-invalid: saga step is malformed")
  end
  if not is_non_empty_string(step.id) then
    error("testkit.saga_conformance: conformance-input-invalid: saga step requires id")
  end
  if not is_non_empty_string(step.effect) then
    error("testkit.saga_conformance: conformance-input-invalid: step " .. tostring(step.id) .. " requires effect")
  end
  if not is_non_empty_string(step.request_queue) then
    error("testkit.saga_conformance: conformance-input-invalid: step " .. tostring(step.id) .. " requires request_queue")
  end
  if type(step.post_conditions) ~= "table" or #step.post_conditions == 0 then
    error("testkit.saga_conformance: conformance-input-invalid: step " .. tostring(step.id) .. " requires non-empty post_conditions")
  end
...
function C.assert_external_effect_saga(saga_def)
  if type(saga_def) ~= "table" then
    error("testkit.saga_conformance: conformance-input-invalid: external-effect saga requires definition")
  end
  if not is_non_empty_string(saga_def.id) then
    error("testkit.saga_conformance: conformance-input-invalid: external-effect saga requires id")
  end
...
local function assert_graphql_add_blocked_by(condition, evidence)
  ...
  for _, forbidden in ipairs(condition.forbidden_fields or {}) do
    if command:find(tostring(forbidden), 1, true) ~= nil then
      error(
        "testkit.saga_conformance: conformance-input-invalid: post_condition " .. tostring(condition.id)
...
function C.assert_external_effect_post_condition(condition, evidence)
  if type(condition) ~= "table" then
    error("testkit.saga_conformance: conformance-input-invalid: external-effect post_condition requires definition")
  end
  ...
  error("testkit.saga_conformance: conformance-input-invalid: unsupported external-effect post_condition kind " .. tostring(condition.kind))
end

function C.assert_progress(_t, case)
  validate_case("assert_progress", case)
  if type(case.first) ~= "function" then
    error("testkit.saga_conformance: conformance-input-invalid: assert_progress requires first")
  end
  local classifier = write_classifier(case)
  if classifier == nil then
    error("testkit.saga_conformance: conformance-input-invalid: assert_progress requires is_write_class")
  end
  ...
  if count_write_calls(before, classifier) + count_raises(result) == 0 then
    error("testkit.saga_conformance: conformance-input-invalid: assert_progress observed no write-class commands")
  end
end
...
function C.assert_idempotent(_t, case)
  ...
  if type(case.first) ~= "function" then
    error("testkit.saga_conformance: conformance-input-invalid: assert_idempotent requires first")
  end
  if type(case.second) ~= "function" then
    error("testkit.saga_conformance: conformance-input-invalid: assert_idempotent requires second")
  end
  local classifier = write_classifier(case)
  if classifier == nil then
    error("testkit.saga_conformance: conformance-input-invalid: assert_idempotent requires is_write_class")
  end
  ...
  if first_effects == 0 then
    error("testkit.saga_conformance: conformance-input-invalid: assert_idempotent: first delivery made no write-class effect; nothing to prove")
  end
  ...
  if second_effects ~= 0 then
    error("testkit.saga_conformance: conformance-input-invalid: assert_idempotent observed effects on second delivery")
  end
end
