...
- `testing-runner.module_test_request`
  ...
  - required: `module`
  - optional: `backend`, `config`, `e2e_driver`, `no_browser`, `dry_run`, `dry_run_github`, `native_argv`, `priority`, `mutation_policy`, `preflight_result`, `artifact_root`, `agentic_testing_repo_root`, `source_ref`, `trace_id`, `dedup_key`
...
When `preflight_result` comes from `browser-readiness.result.v1`, it may include a bounded `request_context` copied from the original readiness check. This context is only for restoring host-provided execution intent after readiness gating. Supported fields are `native_argv`, `dry_run`, `no_browser`, `priority`, and `mutation_policy`; unknown fields must be rejected by the readiness package. `request_context` must remain small control metadata and pointer-only safety evidence, and must not carry reports, evidence bodies, browser state, credentials, cookies, or tokens.
...
`mutation_policy` is optional bounded control metadata for P2 native execution, with schema `testing-runner.mutation-policy.v1`. It contains `priority` and up to eight `actions`; each action has `action`, `target`, and optional pointer fields `evidence_path`, `fixture_path`, `cleanup_path`, and `rollback_path`. The only executable mutation actions are `create_test_data` and `edit_test_data` against `local_test_data`, and each requires `evidence_path` plus at least one of `fixture_path`, `cleanup_path`, or `rollback_path`. `delete`, permissions, billing, external notification, real-user-impacting, unknown, and non-local actions are blocked by default. Native module execution with `priority = "P2"` and no `mutation_policy` returns `blocked` before executing.
...
`fkst-native` metadata may include `native_summary` for implemented native paths. Online heartbeat summaries use `testing-runner.online-heartbeat-summary.v1`, include only small status fields and a sanitized `target` without query or fragment text. Module no-browser summaries use `testing-runner.module-no-browser-summary.v1`, include only the module name, mode, and status. Browser driver summaries use `testing-runner.browser-driver-summary.v1`, include only module, driver, mode, status, and optional bounded readiness audit fields (`status` plus per-session role/status). Mutation policy summaries use `testing-runner.mutation-policy-summary.v1`, include the policy decision, classification, reason, action count, and pointer-only action evidence. Missing browser `native_argv` produces a `planned` readiness-gated plan; provided browser `native_argv` runs directly through `fkst-native` and maps exit code `0` to `passed`, non-zero/unavailable execution to `failed`.

Blocked mutation policy summaries classify unsafe or unsupported side effects as `NOT_EXECUTED_RISK`, and missing fixture, cleanup, or rollback support as `FIXTURE_GAP`. These blocked P2 outcomes are execution risk or fixture/data gaps, not product defects. Executed safe mutations record `SAFE_MUTATION_ALLOWED` with evidence and cleanup or rollback pointers.
