...
- `http`: explicit method, credential-free query-free URL, empty inline headers, timeout, and `status-code`, `body-contains`, or `json-path-equals` assertions.

`json-path-equals` publishes the versioned capability `testing-runner.http-json-path-equals.v1` and is deliberately narrower than a general JSONPath evaluator. Its `path` is a non-empty dot-delimited sequence of ASCII object-member names (`status` or `service.status`), and `expected` must be a bounded string, an IEEE-754 safe integer, or a boolean. Arrays, indexes, wildcards, filters, recursive descent, fractional or unsafe numbers, JSON null, and implicit type coercion are rejected. The response must decode as a JSON object; invalid JSON, a missing member, a type mismatch, or an unequal scalar fails the assertion. JSON decoding uses a non-persisting Host port; the testing runner, not the decoder, compares the typed value and owns the verdict.
...
The host installs `structured_execution_runtime` with authenticated artifact loading, current time, grant verification, atomic replay claim/completion, owner-bound local direct argv and bounded loopback HTTP effects, artifact writing, and completed-result loading. `testing_runtime.structured_execution.production()` and `libraries/testing_runtime/bin/fkst-structured-execution-runtime.js` provide the reference physical-host implementation. The package adds JSON decoding locally without creating a second runtime effect or persistent IPC artifact. It does not create sandboxes, resolve secrets, invoke shells, or weaken Environment Factory cleanup.
