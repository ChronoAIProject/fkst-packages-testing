...
def assert_rejection(result: subprocess.CompletedProcess[str], stage_log: Path, message: str, allowed: tuple[str, ...]) -> None:
    ...
    assert observed == allowed, (message, observed)
...
def signed_case(root: Path, mutate, *, target: str, message: str, allowed: tuple[str, ...]) -> None:
    ...
    result = run_verifier(pin, authorization=authorization, paths=paths, stage_log=log, success=False)
...
def assert_rejection_matrix() -> None:
    ...
    cli = [
        (["--release", str(ARTIFACTS["release"])], "--trusted-authorization-sha256 is required exactly once"),
        (["--trusted-authorization-sha256", legitimate_pin.upper()], "exactly 64 lowercase hexadecimal"),
        (["--trusted-authorization-sha256", legitimate_pin[:-1]], "exactly 64 lowercase hexadecimal"),
        (["--trusted-authorization-sha256", "g" * 64], "exactly 64 lowercase hexadecimal"),
        (["--trusted-authorization-sha256", legitimate_pin, "--trusted-authorization-sha256", legitimate_pin], "arguments must be unique"),
        (["--trusted-authorization-sha256", "--release", str(ARTIFACTS["release"])], "arguments must be unique"),
        (["--trusted-authorization-sha256", legitimate_pin, "--release", str(ARTIFACTS["release"]), "--release", str(ARTIFACTS["release"])], "arguments must be unique"),
        (["--trusted-authorization-sha256", legitimate_pin, "--unknown", "value"], "unknown argument"),
    ]
    ...
    with tempfile.TemporaryDirectory(prefix=".testing-package-release-matrix-", dir=ROOT) as directory:
        ...
        result = run_verifier(hashlib.sha256(authorization_bytes).hexdigest(), authorization=auth_path, paths=paths, stage_log=log, success=False)
