...
local function origin_config(value)
  if not bounded_field(value, max_string) then return nil end
  local scheme, authority = value:match("^(https?)://([^/%?#]+)$")
  if scheme == nil or authority == nil or authority == "" then return nil end
  if authority:find("%s") ~= nil or authority:find("\\", 1, true) ~= nil then return nil end
...
local function configured_origins(request)
  local base_origin = origin_from_url(request.base_url)
  local origins, seen = {}, {}
  ...
  add(base_origin)
  for _, value in ipairs(request.allowed_origins) do
...
local function confidence_for(value)
  ...
  if evidence_source(value) == "accessibility" or evidence_source(value) == "a11y-visible" then score = score + 1 end
...
function M.validate_request(value)
  ...
  local base_origin = origin_from_url(value.base_url)
  if base_origin == nil then
    error("testing-runner: malformed-request: module_discovery.base_url must be an http URL")
  end
  if not dense_list(value.allowed_origins) or #value.allowed_origins == 0 or #value.allowed_origins > 16 then
    error("testing-runner: malformed-request: module_discovery.allowed_origins must be a non-empty dense list")
  end
  local includes_base_origin = false
  for _, origin in ipairs(value.allowed_origins) do
    local allowed_origin = origin_config(origin)
    if allowed_origin == nil then
      error("testing-runner: malformed-request: module_discovery.allowed_origins items must be http origins")
    end
    if allowed_origin == base_origin then includes_base_origin = true end
  end
  if not includes_base_origin then
    error("testing-runner: malformed-request: module_discovery.allowed_origins must include the base_url origin")
  end
...
function M.inventory(value, artifact_root, opts)
  local request = M.validate_request(value)
  local allowed_origins, allowed_origin_map, base_origin = configured_origins(request)
  local base_scope = base_scope_from_url(request.base_url)
  local modules, rejected_count = {}, 0
  opts = opts or {}
  local degraded = opts.readiness == nil or opts.readiness.status ~= "ready"
  if degraded then
  ...
  return {
    ...
    base_url = strip_url_detail(request.base_url),
    allowed_origins = allowed_origins,
    modules = modules,
    module_count = #modules,
    limitations = limitations_for(request, rejected_count, #modules),
    provenance = {
      schema = "testing-runner.module-inventory-provenance.v1",
      source = "module-discovery-request",
      adapter = "fkst-native",
...
function M.summary(inventory, artifact_root, status)
  return {
