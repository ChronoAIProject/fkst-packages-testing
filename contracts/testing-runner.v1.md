...
`cdp_execution.ai_generation` uses `testing-runner.ai-case-generation.request.v1`. Supported modes are `disabled` and `autonomous-reviewed`; only `autonomous-reviewed` may enter the AI authoring and review loop. The request may set a bounded `case_budget`, bounded `allowed_case_kinds`, bounded `allowed_action_kinds`, and safe artifact pointers for `context_manifest_path`, `generated_cases_path`, `generated_case_gate_path`, `ai_agent_generation_path`, `generated_case_agent_review_path`, and `ai_test_design_loop_path`. If these pointers are omitted, the runner derives them under the module `artifact_root`. The generated AI context manifest may additionally bind one validated `testing-design.context-reference.v1`; this carries only the repository-analysis, requirements-index, and traceability-seed artifact pointers plus SHA-256 digests and participates in the context input digest. Inline generated cases, inline model transcripts, raw prompts/responses, screenshots, browser storage, credentials, cookies, tokens, report bodies, and browser state are malformed; generated cases and reviews move only through artifact pointers plus digest binding.
