...
The design module start may carry `ai_design_loop_request` and `ai_design_loop_state_ref` as top-level siblings of `module_discovery` and `cdp_execution`. At browser readiness, workflow-QA attaches its persisted seed artifact reference to `design_module_start.ai_design_loop_request.seed_cases_ref` for non-CDP planning, while preserving the legacy nested CDP attachment path. Both forms use the same bounded pointer/digest request grammar; simultaneous top-level and nested reviewed-state authorities fail closed.
...
The generic reference Host persists the immutable request, workflow state, artifacts, approval and executor replay claims, terminal record, and a Host-wide run index behind the existing runtime ports. Supervisor startup scans that index and emits bounded internal run-scoped ticks so one invalid run cannot discard healthy recovery actions; trigger files carry only Host coordinates and never identify one run. `workflow-qa.redrive` reconstructs each nonterminal run and routes an indexed request without state through the authoritative start path. The process-recovery fixture terminates a real supervisor after the executor completion is durable but before workflow observation, then verifies that a fresh supervisor completes finalization without repeating the effect.
