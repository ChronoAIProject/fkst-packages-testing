...
function M.run(context, project_root)
  ...
  local function release_checkpoint(stage, actions)
    ...
    local receipt = publication.acknowledge_comment(
      checkpoint_written(prepared, comment_id), context.publication_runtime)
    return workflow.handle_publication_receipt(receipt, context.request, context.workflow_runtime)
  end

  local actions = release_checkpoint("intake", workflow.start(context.request, context.workflow_runtime))
  ...
  comment_id = comment_id + 1
  local aggregate_receipt = publication.acknowledge_comment(
    checkpoint_written(prepared, comment_id), context.publication_runtime)
