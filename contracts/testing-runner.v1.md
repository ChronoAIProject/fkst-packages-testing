...
`cdp_execution` is optional for module requests only and uses `testing-runner.module-cdp-execution.v1`. It requires the existing `ui_loop` scope and never redefines runtime scope. It may contain only bounded controller fields: optional integer `step_budget` from 1 to 32, optional dense `case_priorities` containing `P0`/`P1`, and optional dense `stop_conditions`. The controller reuses an already-ready local browser/CDP session reported by readiness; it must not perform credential login, MFA, CAPTCHA handling, write flows, or mutation fixtures. Unknown fields and inline screenshots, traces, browser storage, credentials, cookies, tokens, report bodies, or browser state are malformed.
