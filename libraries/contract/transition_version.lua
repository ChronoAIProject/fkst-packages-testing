...
local function pad_numeric_runs(text)
  return tostring(text or ""):gsub("%d+", padded_order_number)
end
...
local function source_ref_order_key(version)
  local text = tostring(version or "")
  local rest = text
  ...
  if timestamp ~= nil then
    ...
    return timestamp:gsub(":", "-") .. "/loop/" .. padded_order_number(loop_n) .. pad_numeric_runs(suffix_tie)
  end
...
local function version_primary_key_from_parsed(parsed)
  ...
  return 0, source_ref_order_key(base)
end
...
function V.version_order_key(version)
  return source_ref_order_key(version)
end
