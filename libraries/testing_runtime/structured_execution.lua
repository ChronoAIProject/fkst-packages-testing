  ...
  return runtime_client.new({
    ...
    validate_runtime_config = function(value, context)
      ...
      if value.ref == root or value.ref:sub(1, #root + 1) == root .. "/" then
        error("testing-runtime: structured-execution-runtime-config-inside-run-root: runtime config must be outside the run root")
      end
    end,
...
local function call_cli(name, payload, timeout, options, transport_root)
  ...
  if root == nil then
    error("testing-runtime: structured-execution-run-root-missing: request must identify a run root")
  end
  ...
  return {
    sha256_bytes = function(bytes)
      if type(bytes) ~= "string" or #bytes > max_hash_bytes then
        error("testing-runtime: structured-execution-sha256-bytes-invalid: bytes must be bounded")
      end
      ...
      if type(result) ~= "table" or type(result.sha256) ~= "string"
        ...
        error("testing-runtime: structured-execution-sha256-bytes-result-invalid: runtime returned an invalid digest")
      end
