...
local spec = {
  ...
  produces = {
    "test-publication.qa_checkpoint_request", "test-publication.defect_publication_request",
...
local function act(event)
  ...
  log.info("workflow-qa dept=execution tag=PUBLISH actions=" .. tostring(#raised))
  actions.raise_all(raised)
end

return saga.department(spec, { accept = accept, done = function() return false end, act = act, name = "execution" })
