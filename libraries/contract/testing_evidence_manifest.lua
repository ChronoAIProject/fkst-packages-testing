...
local function relative_run_pointer(value, run_id, field)
...
function M.validate(value, result_set, sha256_fn)
  ...
  if result_set ~= nil then
    ...
    local function check_refs(refs, owner)
      ...
      for _, ref in ipairs(refs) do
        ...
        if ref.kind ~= "evidence" or not seen[ref.ref] then fail("missing-entry", owner .. " references an unknown evidence entry") end
        referenced[ref.ref] = true
      end
    end
    for _, case in ipairs(result_set.cases) do
      check_refs(case.evidence_refs, "case " .. case.case_id)
      for _, observation in ipairs(case.observations) do check_refs(observation.evidence_refs, "observation " .. observation.observation_id) end
      for _, assertion in ipairs(case.assertions) do check_refs(assertion.evidence_refs, "assertion " .. assertion.assertion_id) end
    end
  end
...
function M.canonicalize(value) M.validate(value); return canonical_json(value) end
function M.sha256(value, sha256_fn) if type(sha256_fn) ~= "function" then fail("missing-sha256", "a host-supplied SHA-256 function is required") end; M.validate(value); local ok, result = pcall(sha256_fn, canonical_json(value)); if not ok then fail("sha256-failed", "the host SHA-256 function failed") end; digest(result, "sha256 result"); return result end
return M
