...
cmd_test_affected() {
  local paths path pkg broad=0 affected_file
  paths="$(changed_paths)"
  if [ -z "$paths" ]; then
  ...
  affected_file="${TEST_RT:-${TMPDIR:-/tmp}}/fkst-testing-affected.$$"
  : > "$affected_file"
  ...
  local fail=0 affected
  while IFS= read -r affected; do
    echo "test-affected: $affected"
    cmd_test "$affected" || fail=1
  done < "$affected_file"
  rm -f "$affected_file"
  return "$fail"
}
