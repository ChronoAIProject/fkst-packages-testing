...
- `testing-runner.module_test_request`
  ...
  - required: `module`
  - optional: `backend`, `config`, `e2e_driver`, `no_browser`, `dry_run`, `dry_run_github`, `native_argv`, `browser_exploration`, `preflight_result`, `artifact_root`, `agentic_testing_repo_root`, `source_ref`, `trace_id`, `dedup_key`
...
`browser_exploration` is optional for module requests only and represents an already-planned safe P0/P1 browser exploration. It is not a planner surface. The native adapter enforces `allowed_origins`, `step_budget`, `module_boundary`, `stop_conditions`, P0/P1 priorities, and an existing ready browser session from `preflight_result`; login, MFA, CAPTCHA, non-local origins, missing sessions, missing evidence pointers, or unreliable harness action results return `blocked` with boundary/session/harness classifications instead of product defects.
...
For bounded browser exploration, the executable path is `backend = "fkst-native"`, `dry_run = false`, a module request, `e2e_driver` containing a browser-harness/CDP driver identifier, and bounded `browser_exploration`. The adapter reuses the ready session reported by `preflight_result`, delegates each action through the existing browser harness/CDP action seam, stops at the configured step budget or stop condition, records intent/action/target/URL/observation/evidence pointer per action, and blocks boundary/session/harness failures without labeling them as product failures.
