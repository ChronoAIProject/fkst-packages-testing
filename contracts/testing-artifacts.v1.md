...
`native_summary` is diagnostic metadata, not a downstream consumption requirement. Accepted nested native summaries are bounded `testing-runner.module-no-browser-summary.v1`, `testing-runner.module-ui-loop-summary.v1`, `testing-runner.module-inventory-summary.v1`, `testing-runner.module-cdp-execution-summary.v1`, `testing-runner.online-heartbeat-summary.v1`, and `testing-runner.browser-driver-summary.v1` payloads. Module UI loop summaries may include only `schema`, `module`, `status`, `classification`, `mode`, `artifact_root`, `metadata_path`, optional `evidence_bundle_path`, and optional bounded `gap_ref`/`backlog_ref`; `metadata_path` must equal `<artifact_root>/metadata.json`, and `evidence_bundle_path`, when present, must equal `<artifact_root>/evidence-bundle.json`. Module inventory summaries may include only `schema`, `module`, `status`, `discovery_status`, `artifact_root`, `inventory_path`, optional `feature_inventory_path`/`test_plan_path`/`plan_status`, optional `evidence_bundle_path`, `module_count`, and `coverage`; artifact paths must equal `<artifact_root>/module-inventory.json`, `<artifact_root>/feature-inventory.json`, `<artifact_root>/test-plan.json`, and `<artifact_root>/evidence-bundle.json`, and detailed module lists, feature inventory, test cases, and evidence stay in those artifact files. Module CDP execution summaries may include only `schema`, `module`, `status`, `execution_status`, `classification`, `mode`, `artifact_root`, `execution_path`, `metadata_path`, optional `test_plan_path`/`cdp_readiness_ref`, optional `evidence_bundle_path`, and `action_count`; artifact paths must equal `<artifact_root>/cdp-execution.json`, `<artifact_root>/metadata.json`, optionally `<artifact_root>/test-plan.json`, and `<artifact_root>/evidence-bundle.json`, while detailed action records and evidence stay in artifact files. Browser driver summaries may include only a readiness audit: `readiness.status` plus up to 16 session `{ role, status }` entries. Readiness `checks`, report bodies, evidence bundle bodies, module inventories, feature inventories, test plans, CDP execution actions, browser state, credentials, cookies, tokens, screenshots, traces, and arbitrary nested trees must be rejected.
