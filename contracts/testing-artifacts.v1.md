...
`native_summary` is diagnostic metadata, not a downstream consumption requirement. Accepted nested native summaries are bounded `testing-runner.module-no-browser-summary.v1`, `testing-runner.online-heartbeat-summary.v1`, `testing-runner.browser-driver-summary.v1`, and `testing-runner.module-inventory-summary.v1` payloads. Browser driver summaries may include only a readiness audit: `readiness.status` plus up to 16 session `{ role, status }` entries. Module inventory summaries may include only `status`, `discovery_status`, `inventory_path`, `module_count`, and `coverage`; detailed modules and evidence pointers stay in the referenced `module-inventory.json` artifact. Readiness `checks`, embedded module inventories, report bodies, browser state, credentials, cookies, tokens, screenshots, traces, and arbitrary nested trees must be rejected.
