...
- `testing-runner.module_test_request`
  ...
  - required: `module`
  - optional: `backend`, `config`, `e2e_driver`, `no_browser`, `dry_run`, `dry_run_github`, `native_argv`, `ui_loop`, `module_discovery`, `cdp_execution`, `preflight_result`, `artifact_root`, `agentic_testing_repo_root`, `source_ref`, `trace_id`, `dedup_key`
...
`cdp_execution` is optional for module requests only and uses `testing-runner.module-cdp-execution.v1`. It requires the existing `ui_loop` scope and never redefines runtime scope. It may contain only bounded controller fields: optional integer `step_budget` from 1 to 32, optional dense `case_priorities` containing `P0`/`P1`/`P2`, optional dense `stop_conditions`, and optional bounded `mutation_fixtures`. Mutation fixtures are small pointer-only records keyed by `case_id`; they may describe only `create-test-data` or `edit-test-data` safe local fixtures, or default-blocked risk kinds (`delete`, `permissions`, `billing`, `external-notification`, `real-user-impact`). Safe mutation execution requires `ui_loop.mutation_policy` other than `read-only`, a fixture pointer, evidence pointer, and cleanup or rollback pointer; otherwise the P2 case remains a fixture/data gap and is not executed. The controller reuses an already-ready local browser/CDP session reported by readiness; it must not perform credential login, MFA, CAPTCHA handling, destructive changes, externally visible mutations, or real-user-impacting actions. Unknown fields and inline screenshots, traces, browser storage, credentials, cookies, tokens, report bodies, or browser state are malformed.
