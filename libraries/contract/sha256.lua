-- contract.sha256: dependency-free FIPS 180-4 SHA-256 for content identity.
local S = {}

local mask32 = 0xffffffff
local initial_state = {
  0x6a09e667,
  0xbb67ae85,
  0x3c6ef372,
  0xa54ff53a,
  0x510e527f,
  0x9b05688c,
  0x1f83d9ab,
  0x5be0cd19,
}
local round_constants = {
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
  0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
  0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
  0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
  0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
  0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
  0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
  0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
  0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
}

local function rotate_right(value, bits)
  value = value & mask32
  return ((value >> bits) | (value << (32 - bits))) & mask32
end

local function add32(...)
  local sum = 0
  for index = 1, select("#", ...) do
    local value = select(index, ...)
    sum = (sum + value) & mask32
  end
  return sum
end

local function padded(value)
  local bit_length = #value * 8
  local parts = {
    value,
    string.char(0x80),
    string.rep("\0", (56 - ((#value + 1) % 64)) % 64),
  }
  for shift = 56, 0, -8 do
    parts[#parts + 1] = string.char((bit_length >> shift) & 0xff)
  end
  return table.concat(parts)
end

function S.hex(value)
  if type(value) ~= "string" then
    error("contract.sha256: sha256-input-not-string: input must be a string")
  end

  local state = { table.unpack(initial_state) }
  local data = padded(value)
  for chunk_start = 1, #data, 64 do
    local words = {}
    for index = 0, 15 do
      local offset = chunk_start + index * 4
      local first, second, third, fourth = data:byte(offset, offset + 3)
      words[index + 1] = ((first << 24) | (second << 16) | (third << 8) | fourth) & mask32
    end
    for index = 17, 64 do
      local first = words[index - 15]
      local second = words[index - 2]
      local sigma0 = rotate_right(first, 7) ~ rotate_right(first, 18) ~ (first >> 3)
      local sigma1 = rotate_right(second, 17) ~ rotate_right(second, 19) ~ (second >> 10)
      words[index] = add32(words[index - 16], sigma0, words[index - 7], sigma1)
    end

    local a, b, c, d, e, f, g, h = table.unpack(state)
    for index = 1, 64 do
      local sigma1 = rotate_right(e, 6) ~ rotate_right(e, 11) ~ rotate_right(e, 25)
      local choose = (e & f) ~ ((~e) & g)
      local temp1 = add32(h, sigma1, choose, round_constants[index], words[index])
      local sigma0 = rotate_right(a, 2) ~ rotate_right(a, 13) ~ rotate_right(a, 22)
      local majority = (a & b) ~ (a & c) ~ (b & c)
      local temp2 = add32(sigma0, majority)

      h = g
      g = f
      f = e
      e = add32(d, temp1)
      d = c
      c = b
      b = a
      a = add32(temp1, temp2)
    end

    state[1] = add32(state[1], a)
    state[2] = add32(state[2], b)
    state[3] = add32(state[3], c)
    state[4] = add32(state[4], d)
    state[5] = add32(state[5], e)
    state[6] = add32(state[6], f)
    state[7] = add32(state[7], g)
    state[8] = add32(state[8], h)
  end

  return string.format(
    "%08x%08x%08x%08x%08x%08x%08x%08x",
    table.unpack(state)
  )
end

return S
